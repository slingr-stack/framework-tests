...
## Default values (property + `defaultValue` + `onInit()`)
...
export class Ticket extends BaseDataModel {
  ...
  // defaultValue runs on framework initialization paths such as fromJSONWithReferences().
  @TextField({
    maxLength: 30,
    defaultValue: () => {
  ...
  override async onInit(): Promise<void> {
    if (this.displayName === undefined) {
      this.displayName = `${this.status.toUpperCase()}-${this.code}`;
    }
  }
}

// Framework initialization path: property defaults, defaultValue, then onInit.
async function initTicket(): Promise<void> {
  const ticket = await Ticket.fromJSONWithReferences({});
  void ticket;
}
