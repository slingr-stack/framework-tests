...
description: Essential skill for Drumr Framework integration tests. Teaches how to write deterministic backend and frontend integration tests using DrumrIntegrationTestKit without coupling them to E2E-only abstractions.
metadata:
  applies-to:
...
- **Do not force it** into pure React Testing Library assertions if it makes the test less clear. RTL's `screen.getByText()` / `expect(element).toBeInTheDocument()` is often more readable for DOM-level checks.
...
- `initTestContext({ dataSet })` boots the app without binding an HTTP port, then loads `backend/src/dataSets/<datasource-id>/<dataSet>/`.
